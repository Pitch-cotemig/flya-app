...
export async function testSupabaseConnection() {
  try {
    ...
    console.log(
      "🔑 Sessão:",
      data.session ? "Ativa" : "Não autenticado (normal)"
    );
...
export async function listTables() {
  try {
    ...
    const { data, error } = await supabase.rpc("get_public_tables"); // Você precisará criar esta função no Supabase

    if (error) {
      console.log(
        "ℹ️ Para listar tabelas, você precisa configurar uma função RPC no Supabase"
      );
      return [];
    }
    ...
    console.log(
      "ℹ️ Listagem de tabelas requer configuração adicional no Supabase"
    );
    return [];
  }
}
